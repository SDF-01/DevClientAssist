---
name: database-schema-change-protocol
description: "Use this skill to evaluate schema changes for data integrity, compatibility, migrations, indexes, and API impact."
---

# Schema Change Protocol Skill

## Purpose

Use this skill to evaluate schema changes for data integrity, compatibility, migrations, indexes, and API impact.

This skill is the operational workflow that supports the related Cursor rule:

```text
.cursor/rules/database/schema-change-protocol.mdc
```
...
Use this skill when the user request, changed files, or related rule indicates that evaluate schema changes for data integrity, compatibility, migrations, indexes, and API impact.
...
1. Classify the change as additive, compatible, breaking, or destructive.
2. Prefer additive migrations and staged rollouts.
3. Review generated SQL or migration files before execution.
4. Do not run destructive migration commands without explicit user approval.
5. Inspect schema, migrations, models, relationships, indexes, and seed data.
6. Assess data loss, backward compatibility, and rollout impact before changing schema.
7. Prefer additive migrations and reversible changes.
8. Validate generated migration files before running them.
9. Run ORM validation and query tests when available.
