---
name: deployment-project-connections-required
description: "Use this skill to read project connection metadata before pushing, deploying, aliasing, or touching external services."
---

# Project Connections Required Skill

## Purpose

Use this skill to read project connection metadata before pushing, deploying, aliasing, or touching external services.

This skill is the operational workflow that supports the related Cursor rule:

```text
.cursor/rules/deployment/project-connections-required.mdc
```
...
Use this skill when the user request, changed files, or related rule indicates that read project connection metadata before pushing, deploying, aliasing, or touching external services.

Also use it when the user manually invokes the skill by name:

```text
/deployment-project-connections-required
```
...
1. Confirm the configured repository, branch, provider, production URL, and deploy mode.
2. Do not manually deploy when GitHub integration is configured to handle deployment.
3. Do not treat random preview URLs as production.
4. Read `.cursor/project-connections.md` before touching remotes, hosting, or external services.
5. Inspect current branch, remotes, workflow files, and deployment config.
6. Avoid production changes unless explicitly required and safe.
7. Verify build and CI behavior before pushing or deploying.
8. Report branch, commit, provider, production URL, and deployment expectation.
