---
name: testing-regression-test-rule
description: "Use this skill to add a focused test that proves a fixed bug does not return."
---

# Regression Test Rule Skill

## Purpose

Use this skill to add a focused test that proves a fixed bug does not return.

This skill is the operational workflow that supports the related Cursor rule:

```text
.cursor/rules/testing/regression-test-rule.mdc
```
...
Use this skill when the user request, changed files, or related rule indicates that add a focused test that proves a fixed bug does not return.

Also use it when the user manually invokes the skill by name:

```text
/testing-regression-test-rule
```
