---
name: global-dependency-control
description: "Use this skill to control package installation, dependency updates, lockfiles, and framework additions so the agent does not introduce unnecessary or unsafe dependencies."
---

# Dependency Control Skill

## Purpose

Use this skill to control package installation, dependency updates, lockfiles, and framework additions so the agent does not introduce unnecessary or unsafe dependencies.

This skill is the operational workflow that supports the related Cursor rule:

```text
.cursor/rules/global/dependency-control.mdc
```
...
Use this skill when the user request, changed files, or related rule indicates that control package installation, dependency updates, lockfiles, and framework additions so the agent does not introduce unnecessary or unsafe dependencies.

Also use it when the user manually invokes the skill by name:

```text
/global-dependency-control
```
