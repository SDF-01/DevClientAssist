...
description: "Use this skill to summarize shipped changes, risks, migrations, user impact, and rollback notes."
---
...
Use this skill to summarize shipped changes, risks, migrations, user impact, and rollback notes.

This skill is the operational workflow that supports the related Cursor rule:

```text
.cursor/rules/deployment/release-notes.mdc
```
...
Use this skill when the user request, changed files, or related rule indicates that summarize shipped changes, risks, migrations, user impact, and rollback notes.
...
1. Group changes into features, fixes, breaking changes, migrations, known issues, and verification.
2. Write notes for the user impact, not just internal file changes.
3. Read `.cursor/project-connections.md` before touching remotes, hosting, or external services.
4. Inspect current branch, remotes, workflow files, and deployment config.
5. Avoid production changes unless explicitly required and safe.
6. Verify build and CI behavior before pushing or deploying.
7. Report branch, commit, provider, production URL, and deployment expectation.
