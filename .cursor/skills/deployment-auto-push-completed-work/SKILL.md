---
name: deployment-auto-push-completed-work
description: "Use this skill to commit and push completed work to the configured GitHub remote after verification and safety checks."
---

# Auto Push Completed Work Skill

## Purpose

Use this skill to commit and push completed work to the configured GitHub remote after verification and safety checks.

This skill is the operational workflow that supports the related Cursor rule:

```text
.cursor/rules/deployment/auto-push-completed-work.mdc
```
...
Use this skill when the user request, changed files, or related rule indicates that commit and push completed work to the configured GitHub remote after verification and safety checks.

Also use it when the user manually invokes the skill by name:

```text
/deployment-auto-push-completed-work
```
...
1. Read `.cursor/project-connections.md` before committing or pushing.
2. Run available verification commands before committing.
3. Stage only files related to the completed task.
4. Use a conventional commit message that explains the value of the change.
5. Push with `git push` or `git push -u origin HEAD` when upstream is missing.
6. Report branch, commit hash, remote, checks, and deployment expectation.
7. Read `.cursor/project-connections.md` before touching remotes, hosting, or external services.
8. Inspect current branch, remotes, workflow files, and deployment config.
9. Avoid production changes unless explicitly required and safe.
10. Verify build and CI behavior before pushing or deploying.
11. Report branch, commit, provider, production URL, and deployment expectation.
