---
name: frontend-design-system-enforcement
description: "Use this skill to use existing tokens, primitives, typography, spacing, color, and layout conventions instead of ad hoc styling."
---
...
Use this skill to use existing tokens, primitives, typography, spacing, color, and layout conventions instead of ad hoc styling.

This skill is the operational workflow that supports the related Cursor rule:

```text
.cursor/rules/frontend/design-system-enforcement.mdc
```
...
Use this skill when the user request, changed files, or related rule indicates that use existing tokens, primitives, typography, spacing, color, and layout conventions instead of ad hoc styling.
...
1. Inspect existing components, routes, styles, tokens, and UI primitives.
2. Find the closest existing pattern before adding a new one.
3. Implement with typed props, semantic markup, accessible interactions, and responsive behavior.
4. Check loading, empty, error, success, hover, focus, and disabled states.
5. Run build, lint, typecheck, and component tests when available.
