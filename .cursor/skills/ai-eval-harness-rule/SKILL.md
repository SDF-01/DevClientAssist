---
name: ai-eval-harness-rule
description: "Use this skill to create evaluation prompts, golden cases, scoring criteria, and regression tests for AI features."
---

# Eval Harness Rule Skill

## Purpose

Use this skill to create evaluation prompts, golden cases, scoring criteria, and regression tests for AI features.

This skill is the operational workflow that supports the related Cursor rule:

```text
.cursor/rules/ai/eval-harness-rule.mdc
```
...
Use this skill when the user request, changed files, or related rule indicates that create evaluation prompts, golden cases, scoring criteria, and regression tests for AI features.

Also use it when the user manually invokes the skill by name:

```text
/ai-eval-harness-rule
```
...
1. Create golden cases for happy path, edge cases, adversarial inputs, and expected refusals.
2. Use repeatable scoring criteria and record baseline outputs before changes.
3. Identify model inputs, retrieved sources, tools, permissions, and output risks.
4. Separate trusted system instructions from untrusted external content.
5. Ground claims in provided or retrieved sources when required.
6. Add evaluations for important model behavior and failure modes.
7. Document limits, fallbacks, and safety boundaries.
