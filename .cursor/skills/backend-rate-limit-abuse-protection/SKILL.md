---
name: backend-rate-limit-abuse-protection
description: "Use this skill to protect public, auth, mutation, upload, and expensive endpoints from abuse."
---

# Rate Limit Abuse Protection Skill

## Purpose

Use this skill to protect public, auth, mutation, upload, and expensive endpoints from abuse.

This skill is the operational workflow that supports the related Cursor rule:

```text
.cursor/rules/backend/rate-limit-abuse-protection.mdc
```
...
Use this skill when the user request, changed files, or related rule indicates that protect public, auth, mutation, upload, and expensive endpoints from abuse.

Also use it when the user manually invokes the skill by name:

```text
/backend-rate-limit-abuse-protection
```
...
1. Identify expensive, public, auth, mutation, and upload endpoints.
2. Apply rate limits, request size limits, abuse logging, and user-safe error messages.
3. Inspect existing service, route, middleware, validation, and error patterns.
4. Define request, response, auth, validation, and failure behavior before coding.
5. Implement boundary validation and server-side authorization.
6. Add useful logging without secrets or sensitive data.
7. Run server tests, lint, typecheck, and build checks when available.
